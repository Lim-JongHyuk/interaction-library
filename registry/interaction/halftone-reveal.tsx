...
export function HalftoneReveal({ src = DEFAULT_SRC, inkColor = "#141414", paperColor = "#fff7e6", mode = "mono", dotSize = 1, dotDensity = 71, angle = 45, shape = "circle", contrast = 1.15, invert = false, revealRadius = 0.4, edge = 0.8, follow = 0.37, idleReveal = 0, trigger = "hover", borderRadius = "16px", className = "", style }: HalftoneRevealProps) {
  ...
  useEffect(() => { propsRef.current = { inkColor, paperColor, mode, dotSize, dotDensity, angle, shape, contrast, invert, revealRadius, edge, follow, idleReveal, trigger }; }, [inkColor, paperColor, mode, dotSize, dotDensity, angle, shape, contrast, invert, revealRadius, edge, follow, idleReveal, trigger]);
  useEffect(() => {
    ...
    const fallback = document.createElement("canvas"); fallback.width = 960; fallback.height = 640; const fallbackContext = fallback.getContext("2d"); if (fallbackContext) { const gradient = fallbackContext.createLinearGradient(0, 0, fallback.width, fallback.height); gradient.addColorStop(0, "#f59e0b"); gradient.addColorStop(.5, "#ec4899"); gradient.addColorStop(1, "#312e81"); fallbackContext.fillStyle = gradient; fallbackContext.fillRect(0, 0, fallback.width, fallback.height); fallbackContext.fillStyle = "rgba(255,255,255,.28)"; for (let i = 0; i < 42; i++) { fallbackContext.beginPath(); fallbackContext.arc((i * 137) % fallback.width, (i * 83) % fallback.height, 18 + (i % 7) * 9, 0, Math.PI * 2); fallbackContext.fill(); } texture.image = fallback; uniforms.uImageSize.value = [fallback.width, fallback.height]; }
  ...
  // The WebGL scene remains mounted while props are synchronized through propsRef in the animation loop.
